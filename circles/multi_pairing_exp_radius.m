...
[x_c_T,y_c_T] = sparse_c(10,stopx,stopy,2*ri);
%x_c_T = [3 6 9 3 6 9];
%y_c_T = [4 4 4 7 7 7];
...
for rad_iter = 1:4
    ...
    for i=1:length(locDev)
        for j=1:length(x_c_T)
            if norm([locDev(1,i) locDev(2,i)] - [x_c_T(j) y_c_T(j)])< 4%(r+r_c)
               C{j}=[C{j} i];
            end
        end
    end
    ...
    if numel(x_range)==1
        x_c_T(x)=closeToDev(1);
        y_c_T(x)=closeToDev(2);
    else
        polyin1=[];
