...
%get the devices that are in the range of C1
x=randi([1,length(x_c_T)]);
x_range = C{x};
closeToDevArray=[];

%find the centroid of C1
...
while sum(chargers_remained==0)~=length(x_c_T)
    ...
    for i=1:length(chargers_remained)
        c_iter=chargers_remained(i);
        if c_iter~=0
            distcc = norm([x_c_T(c_iter) y_c_T(c_iter)]- [x_c_T(x) y_c_T(x)]);
            if distcc < mind     
               mind = distcc;
               x_new = c_iter;
            end
    ...
    for i=1:numel(inter)
         x1 = norm([x_c_T(x) y_c_T(x)]- [inter(1) inter(2)]);
         x2 = norm([x_c_T(x_new) y_c_T(x_new)]- [inter(1) inter(2)]);
         x_mean = x1+x2;
         inter_dist(i) = x_mean;        
    end
