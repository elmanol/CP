...
%     [ linex, liney ] = liner([C_interx(1),C_intery(1)], [C_interx(2),C_intery(2)]);
    ...
    for k=1:numel(linex)
        ...
        if i==2
            ...
            error=zeros(size(intersections,1),2);
        elseif i>=3
            ...
            for c3p = 1:size(dC{inter(i),move},2)
                ...
                for s=1:size(intersections,1)
                   ...
                   if abs(interToDev-r3p)<lambda/4
                        error(s,1)= error(s,1)+1;
                        error(s,2)= error(s,2)+ abs(interToDev-r3p);
                   else
                        %error(s,i)=0;
                   end
                end
        ...
        m=max(error);
        minE=10^3;
        for er=1:length(error)
            if m(1,1)==error(er,1) && error(er,2)<minE
                minE=error(er,2);
                numE=er;
            end
        end   
